...
| <a id="decision-input-combination"></a>Let inputs combine rather than compete. | This principle applied to collected evidence but not to the inputs themselves: an SBOM and a package-manager input were alternatives, so each scan was capped at whatever one of them happened to know, and their disagreements were unobservable. Combining is not a ranking of input kinds: an input that determined a fact owns it in the combined row, and an input that determined nothing does not withdraw it. | [Input combination](specs/cli.md#contract-input-combination), [folded relationship](specs/cli.md#contract-folded-relationship), and [component supply](specs/cli.md#contract-component-supply) |
