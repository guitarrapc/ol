...
### Phase 1: CLI contract tests（Red、完了）
...
### Phase 2: Policy equivalence classes（Red、完了）
...
### Phase 3: 共通scan executionの抽出（Green、完了）
...
### Phase 4: Allow policy evaluator（Green、完了）
...
### Phase 5: `CheckCommands` とtext renderer（Green、完了）
...
- `ScanExecution` がinput/SPDX/cache準備とinventory/enrichmentを共有し、`scan`と`check`が同じcompleted `ScanResult`境界を使う。
- `LicenseAllowPolicy` がallow-listを実行ごとに一度だけ正規化して`FrozenSet<string>`へ格納し、SPDX expressionをUTF-8 span上でBoolean評価する。
- pass pathはviolation resultを割り当てず、violation pathはcomponent indexとkindだけを持つowned arrayを返す。
- `CheckCommands` は全componentを評価し、全violationをdeterministicなtextでstdoutへ出す。framework option parse failureもpolicy violationと区別してexit 2へ写像する。
- policy equivalence classとCLI contractを含むfull test suiteは223件すべて成功した。

1,024 componentsのfocused benchmark baselineは次のとおりである。

| Case | Mean | Allocated |
| --- | ---: | ---: |
| all allowed | 66.05 us | 1 B |
| mixed `AND` / `OR` expressions | 223.36 us | 2 B |
| all violations | 114.05 us | 8,217 B |
| all unresolved | 10.58 us | 8,216 B |

violation/unresolvedのallocationは返却する1,024件のowned violation arrayに対応する。componentごとの一時stringまたはcollection allocationは計測されていない。
