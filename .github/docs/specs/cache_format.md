...
`unpack` enforces the same compressed, expanded, per-entry, and entry-count limits before committing any file. It derives every destination from the recognized category and validated opaque name rather than joining an archive-supplied path. The manifest and every entry are decompressed and validated in memory before replacement begins. `unpack` reads the archive twice from one captured copy of the compressed bytes: once to validate every entry, then, only if all of them passed, once more to write them. No entry's bytes are kept past the entry they belong to, so an archive's expanded content never has to fit in memory at once — what a restore holds instead is the compressed archive, which the archive limit bounds, and per-entry bookkeeping that scales with the entry count rather than with the content. Restoring an archive that expands to the 64 MiB limit measured a peak working set of 16.6 MiB from 32 large entries and 57.8 MiB from ten thousand small ones, against 78.8 MiB and 114.0 MiB for a design that held every entry until the last one had been judged. The second read is what buys that, and it is not free: reading and validating the archive again is work a single pass did not do, and a restore no longer decompresses everything before it starts writing. The cost measured between none and roughly a fifth of the restore, depending on the shape of the archive: it is largest when few, large entries mean the second read does the most work per file written, and disappears when thousands of small writes dominate, against a memory bound that no longer grows with what the archive expands to — which is the property that matters, because the entry-count and expanded-content limits are what an archive is allowed to reach and a restore has to survive reaching them. Inspecting an archive passes over it once and so reads it straight from the file, without the copy. Nothing is written outside the destination categories. Each replacement writes a dot-prefixed temporary file beside its destination and renames it over the entry. Existing symbolic links and reparse points in the cache-root path, category paths, and destination entry paths are rejected, so a pre-existing link cannot redirect a write outside `--cache-dir`. The cache-root and category paths are walked in full before anything is written — once for all three when the run starts, and again for a category the first time an entry is restored into it — and the category directory is rechecked before each temporary file is written into it, and both it and the destination entry are rechecked immediately before each replacement. The archive is neither signed nor authoritative: category readers apply their complete schema validation when an entry is requested, exactly as they do for a locally written cache.
