...
A `metadata.view` that cannot be read is a command failure, and a view stating no filter is not the same document as one stating nothing: the filter field must be present, holding `null` when the scan applied none. A report predating the field is read as unfiltered, which is what it was. Everything else — an absent value, a value of the wrong type, an object with the field missing — would leave `check` unable to tell a complete report from a narrowed one, and the failure this states exists to prevent is exactly that confusion.
