...
Every successful `scan` writes its report to stdout and a labeled summary to stderr, whatever `--format` was asked for. `--quiet` suppresses the human-readable stderr summary, never the stdout result. `--verbose` may add diagnostics to stderr and additional report fields, but does not change the result.

<a id="contract-summary-every-format"></a>

JSON was once exempt, on the reasoning that the document already states everything the summary states and should not be repeated. The document does state it — [`metadata.inputDiscovery`](#contract-input-discovery-metadata) closed the last gap — but the reasoning was still wrong, because the two are read by different people. A caller redirects the report to a file and reads the terminal; whoever reads the terminal cannot open the file. The exemption therefore made the recommended CI path, the one that persists a report for `check`, the only path that left no trace of having run: a job could scan a repository and show nothing in its log. Redirecting stdout is unaffected, because the summary was always on stderr.

The summary is one vocabulary for one run, so the same lines are written whichever report format was requested; only the stdout projection differs. A caller that wants silence has `--quiet`, which is the option that was always the answer to "do not print this". Every counted noun that follows its count takes its plural from that count, including counters that read plural in every ordinary run, because a line that says `1 components` is the kind of detail that makes a reader doubt the numbers beside it. Two shapes are outside that rule. The `License results` status names — `conflict`, `unknown`, `error` and the rest — are the status vocabulary canonical JSON and `check` share, and a status is named rather than counted. And the object of a preposition stays singular however large the count: `2 without purl`, `2 with unknown dependency type`, and `2 components without source license` all count components, and a component has one purl, one relationship, and one source license. In the first two the counted noun is elided, so there is no noun for the count to inflect.

<a id="contract-evidence-table"></a>

The four external-evidence collectors — package artifacts, declared GitHub files, package metadata, source repositories — report their counters as one aligned table under a single `Evidence (full scan)` heading rather than as one labeled line each. They share most of a vocabulary (targets, requests, cache hits and misses, documents, matches, errors) but not all of it, and one line per collector placed the shared counters somewhere different on every line, so comparing one counter across collectors meant re-reading all four. A cell for a counter a collector does not have is `-`, never `0`: a zero claims the collector attempted that work and found nothing, which is the distinction the `External evidence` line already draws for a whole run. `requests` counts GitHub API requests, which is why package metadata — whose fetches go to package registries and which keeps no request counter — carries `-` there even in a row whose cache misses are nonzero. `cache hits` and `cache misses` keep the word `cache` in their headers rather than shortening to `hits` and `misses`, because a column headed `hits` in a table that also has a `matched` column reads as a count of license matches. Counters only one collector has stay in a line named for that collector under the table, so no column exists that would be `-` in most of its rows. Package metadata's `targets` cell counts supported components rather than planned lookups, because that row's cache hits and misses are counted per component and a lookup count would not sum with them; that is also why the same word names a different quantity than `metadata.packageMetadata.targetCount` in canonical JSON, which reports the deduplicated lookups. The values are the ones the labeled lines carried; the table adds, removes, and aggregates nothing.
...
Dependency type is `root`, `direct`, `transitive`, or `unknown`. It is `unknown` whenever the input cannot prove the relationship. The value is required in canonical JSON and shown in the default human-readable ASCII table, whose columns are, in order:
...
<a id="contract-text-table"></a>

Every text table shares one framing. A header row names the columns, a row of `-` runs matching the column widths follows it, and each row states one record. Cells are left-aligned and separated by two spaces; the last column is never padded, so no row carries trailing whitespace. A cell with no value is `-`, never blank. A `\r`, `\n`, or `\t` inside a value becomes a single space, because a value that ends the row would end the record.

A column is as wide as its widest cell, measured in **terminal columns rather than bytes**: an East Asian wide character occupies two, a combining mark occupies none, and an accented Latin character occupies one however many bytes it costs. Width is capped at 256 columns. A cell wider than its column is written in full and overflows, because a component named in a violation has to stay identifiable and only that row is then ragged; without the cap one oversized value — which no registry issues but a hand-written input can still carry — would pad every other row to its length.
...
A status alone does not tell a reviewer what to do next, and the mechanism that left a license unresolved decides that: wait for Ol to gain a capability, open a document, or ask the publisher. So `text` and `markdown` component views follow the table with an `Unresolved components` table containing `NAME VERSION REASON REFERENCE PATH`; both state an absent `REFERENCE` or `PATH` as `-`. The `text` table uses the [shared framing](#contract-text-table).
...
| `ol cache info` | Show the contents of a cache directory or archive. | Cache type, counts, sizes, and the entries that failed validation; `--verbose` adds every managed entry with its logical key and timestamp. |
...
`--format text` is the default and writes the terminal-oriented result described below. `--format markdown` writes one GitHub-flavored Markdown document to stdout in reader order: `## ol license check`, `### Result`, `### Violations`, an optional `### Unresolved mechanisms` tally, `### Resolved license usage`, `### Coverage`, a collapsible `### All components` package table, and `### Diagnostics`. The result appears before the details so a CI reader can immediately see passed, failed, or inconclusive; the violation table retains the package, version, ecosystem, purl, license evidence or status, policy reason, unresolved mechanism, reference, and dependency path. The all-components table includes dependency relationship and input supply, while Coverage combines status, ecosystem, supply, and warning counts. Persisted values escape table separators, line breaks, and the HTML-significant `&`, `<`, and `>` characters so dependency-controlled text cannot alter the document structure. Diagnostics retain the persisted scan input, source, SPDX list, dependency filter, discovery, and excluded input paths. The complete document is written before `check` returns exit `2` or `3`, so a CI step can redirect stdout to `GITHUB_STEP_SUMMARY` without a second summary command or a separate output file.
...
Violations are printed as an ASCII table using the [shared framing](#contract-text-table), with `Package Version Ecosystem Purl License/Status Reason Mechanism Reference Path` columns. `Path` is the [dependency path](#contract-dependency-path) the scan report's inventory proves, stated as `-` when the report names no introducer for that component.
...
Only `unknown`, `ambiguous`, `conflict`, and `invalid` may be acknowledged into a baseline, and only when no recognizable candidate is rejected by the active allow-list. `matched` belongs in the allow-list and `error` represents an operation to repair. Each retained evidence item states its source, kind, raw claim, and any declared license reference kind and value, so the file itself shows where a publisher said an otherwise empty claim can be reviewed. A declared reference remains provenance and never becomes the raw license claim. Entries identify the component and fingerprint its status and all of those evidence values, so changed evidence or identity expires that acknowledgement. Applying a baseline rechecks the active allow-list. Missing, malformed, or incompatible baselines exit `1`; acknowledged counts, including zero, are reported.
...
<a id="contract-diff-input-coverage"></a>

Input coverage states, per report, whether it [declared no resolved dependencies](#contract-empty-inventory) and what [discovery](#contract-input-discovery-metadata) detected, ignored, and skipped. The two above are boundaries the caller drew; this one is what discovery observed, and it belongs beside them because it moves the same thing — which population the comparison covers.

The failure it prevents is that a report which read fewer inputs holds fewer components, and every one of them appears in `changes` as a removal. "An input was not read" and "a dependency was removed" are otherwise the same diff. The empty report is the extreme: it turns the whole other side into removals, and two empty reports compare as no change at all. Ol states the two coverages and draws no conclusion, as with the boundaries beside it.

Human text states the boundary when either side declared no dependencies, ignored a candidate, skipped a companion set, or never stated its discovery, and whenever the two sides differ. That is the same rule the boundaries above follow, applied to a fact whose empty state is different: an exclusion list or a dependency filter is absent when the caller drew no boundary, while a detected-file count never is, so restating an unremarkable count on every diff would add lines readers learn to skip. JSON carries the object unconditionally, as it does the other two.

`changed` is `null`, and human text reads `unknown`, when either report predates `metadata.inputDiscovery`. Such a report still detected input files and merely did not record how many, so answering `no` would assert a comparison Ol never made — which is the one thing a boundary exists to prevent. Excluded input paths are read as none for such a report instead, because an Ol without the option truly excluded none; the difference is whether the absent value is knowable, not how old the document is. Ignored candidates are compared as a set, for the reason the two lists above are.

```text
ol cache clear [package-metadata|source-repository|github-file|all]
ol cache list [--cache-dir <directory>]
ol cache info [<cache-directory-or-archive>] [--format <text|markdown>] [--cache-dir <directory>] [--verbose]
...
`list` resolves the cache locations using the same precedence as `scan` (`--cache-dir`, `OL_CACHE_DIR`, legacy category roots, then the platform user-cache location). It prints each managed category path, the count of hash-named entries, their total file size, and a total. Every one of those answers is a property of the directory rather than of an entry, so `list` never opens an entry: it rejects a link where an entry belongs, and otherwise counts and measures what it enumerated. A corrupt entry is therefore counted and measured like any other, which is what `list` did when it validated too. `info` accepts a managed cache root, one managed category directory, or an `.olcache` archive; when its path is omitted it inspects the resolved cache locations. A positional path and `--cache-dir` cannot be combined. Its default text output uses the [shared framing](#contract-text-table) for the overview, category summaries, and entries. This deliberately places a category's path, and an entry's logical `CacheKey`, on the same row as their counters, which the earlier vertical listing kept on separate lines. Someone inspecting a cache is comparing its parts against each other — which category holds the entries, which one grew, which entry is stale — and a value at a fixed column offset can be read down while a value on its own line cannot. The path and the cache key are the long cells; the framing's column cap bounds only a pathological value, so an ordinary path still widens the row and is accepted as the cost of the comparison. Inspecting an archive reports what the archive states, so it validates each entry as the entry is decompressed and keeps none of them; `unpack` reads the archive a second time to write it, keeping no entry either, and writes each one to a dot-prefixed temporary file beside its destination and renames it over the entry. `--format markdown` emits the same information as Markdown tables, including the compressed archive size for an `.olcache` input. Markdown syntax and raw HTML characters in inspected values are escaped. Its entry table reports the managed entries that failed validation, each with its physical file name in the details column. `--verbose` extends that table to every managed entry, with its category, logical `CacheKey`, UTC `FetchedAt`, and file size. Both orderings are by logical `CacheKey`, then by the physical file name that is unique; an entry too invalid to have a key sorts first. Unknown sibling files are counted but not interpreted. This inspection validates the common transport schema and identity; category-specific evidence validation still occurs when `scan` reads an entry. Explicit cache inspection is the only CLI output that intentionally reveals the resolved cache path.
...
`unpack` treats the archive as untrusted input. It accepts format version `1`, the three managed category names, and lowercase SHA-256 JSON entry names only. Absolute or nested paths, traversal, archive links, non-regular entries, duplicate entries, unsupported versions, malformed cache identity, and archives or entries that exceed the hard limits above are rejected. Existing symbolic links and reparse points in the cache-root path, category paths, or destination entry paths are also rejected. The cache-root and category paths are walked in full before anything is written — once for all three when the run starts, and again for a category the first time an entry is restored into it; the category directory is then rechecked before each temporary file is written into it, and both it and the destination entry are rechecked immediately before each replacement, because those are the paths the write and the replacement resolve. Re-walking every ancestor once per entry is what a restore of a large cache spent its time on, and it bought a narrower window rather than a closed one. All entries are read and validated in memory before managed cache files are replaced, so an archive rejected for its content changes nothing on disk. Replacement itself is per entry rather than transactional: a link found at a destination, or a filesystem failure, rejects the run with the entries before it already replaced. Unrelated files under the isolation root are preserved. Full category-specific validation still occurs when scan reads an entry, so transporting a cache never makes its evidence authoritative.
...
- A table that answers a question only for a cache small enough to read is not a table, and the fix is a filter rather than a cap. `cache info` listed every managed entry, which on an ordinary development machine meant 5,979 rows and 1.1 MB of output for a command whose useful answer was one line. Capping the listing was the obvious repair and the wrong one: entries were ordered by the SHA-256 the file is named after, so the first N rows were a random sample of cache keys, and the same ordering meant the complete listing could not be scanned by eye either. What separated the rows worth printing from the rest was not position but status. The default now lists the entries that failed validation — the one fact `cache list` cannot supply, and therefore the reason `info` exists beside it — and `--verbose` restores the complete listing, now ordered by logical `CacheKey` so that it is searchable. Filtering by what makes a row interesting also removed the argument for a magic N.
- A layout that never crowds a line also never lets two lines be compared. `cache info` printed each value on its own indented line, so a long filesystem path or logical cache key never shared a dense row — and answering "which category holds the entries" meant reading three separate blocks and holding three numbers in mind. Moving to the shared table put each path back on a row with its counters and took the widest line from 156 to 202 columns on an ordinary Windows profile, where the path alone is already 145 of them. That cost was measured and accepted: the values the path now shares a row with are short and drawn from fixed vocabularies — counts, sizes, `valid`/`invalid` — so each sits at a stable column offset the eye can run down, where the vertical form gave a stable offset only within one block. The property worth carrying over was never one value per line; it was that no value is truncated, which the overflow rule keeps.
