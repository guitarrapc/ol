...
`--exclude-input-path` is repeatable and removes existing, exact file or directory paths from directory-input discovery. A relative exclusion is resolved once from the current working directory, using the same path semantics as `--input`; it is not expanded independently beneath each directory input. The resolved exclusion must be a strict descendant of at least one directory input. Missing paths, the input directory itself, and paths outside every directory input are invalid. Matching uses path-segment boundaries and the operating system's path case semantics. Glob syntax is not supported. Naming a file explicitly with `--input` while also placing it beneath an exclusion is an input failure rather than an order-dependent override.
