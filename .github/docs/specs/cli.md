...
Matching is ordinal, case-sensitive, and anchored at purl separators (`/`, `.`, `@`):
...
- a full purl such as `pkg:npm/left-pad@1.3.0` excludes exactly that component;
- a component with no purl is never excluded, and a casing mismatch leaves the component evaluated, because both failures must leave the gate closed rather than open.

Empty entries, an empty list, a value that is not a `pkg:` prefix, and a value naming only an ecosystem such as `pkg:npm/` are invalid check options; a single entry must not be able to remove a whole ecosystem from evaluation. Duplicate prefixes have no additional effect. `--exclude-packages` applies identically to `--report` evaluation, so it changes no report field and needs no report schema support. When supplied, `check` prints `Excluded from evaluation: N components.` — including `0` — so a prefix that stopped matching is visible in CI logs.
...
Report evaluation performs no input parsing, no registry or repository collection, and no network access. Separating policy from collection this way means a policy can be re-run, or a different policy applied, without the result depending on what a registry happened to answer at that moment. `--report` therefore cannot be combined with `--input`, `--input-format`, `--refresh`, `--no-external-evidence`, or `--cache-dir`; combining them is a configuration error. `--exclude-packages`, `--baseline`, `--update-baseline`, `--spdx-data`, and `--sarif` still apply.
