...
`--exclude-input-path` is repeatable and removes existing, exact file or directory paths from directory-input discovery. A relative exclusion is resolved once from the current working directory, using the same path semantics as `--input`; it is not expanded independently beneath each directory input. The resolved exclusion must be a strict descendant of at least one directory input. Missing paths, the input directory itself, and paths outside every directory input are invalid. When exclusions are supplied, inputs and exclusions are resolved together from their common parent using file-system entry casing, then matched ordinally at path-segment boundaries. Glob syntax is not supported. Naming a file explicitly with `--input` while also placing it beneath an exclusion is an input failure rather than an order-dependent override.
...
The human-readable scan summary reports the number of detected physical input files, the known candidates discovery ignored, the companion sets it skipped, and the distinct component ecosystems in ordinal order; a scan with no identified component ecosystem reports `none`.
...
A folded row keeps the relationship its resolver gave it, including `unknown`, wherever the SBOM states `root`. An SBOM's root is the root of the SBOM's own graph, and a package-manager input listing a component is itself the determination that the component is a dependency of the scanned resolution, so a folded root describes the receiving row no better than silence does. The distinction is not cosmetic: [policy skips a root](#contract-policy-checks), so merging the value would let a second input withdraw a resolved dependency from the gate — the same failure that [an abstaining usage occurrence](#contract-development-usage-and-sbom) was ruled out for, in the field beside it. An SBOM root that no package-manager input answers for is unaffected, because no fold happens: it keeps its own row and stays a root, exactly as when the SBOM is scanned alone. Where both inputs state a relationship other than `root`, the combined row still reports the closest one to a root that either input observed.
