...
The summary is one vocabulary for one run, so the same lines are written whichever report format was requested; only the stdout projection differs. A caller that wants silence has `--quiet`, which is the option that was always the answer to "do not print this".

<a id="contract-evidence-table"></a>

The four external-evidence collectors — package artifacts, declared GitHub files, package metadata, source repositories — report their counters as one aligned table under a single `Evidence (full scan)` heading rather than as one labeled line each. They share most of a vocabulary (targets, requests, cache hits and misses, documents, matches, errors) but not all of it, and one line per collector placed the shared counters somewhere different on every line, so comparing one counter across collectors meant re-reading all four. A cell for a counter a collector does not have is `-`, never `0`: a zero claims the collector attempted that work and found nothing, which is the distinction the `External evidence` line already draws for a whole run. Counters only one collector has stay in a line named for that collector under the table, so no column exists that would be `-` in most of its rows. Package metadata's `targets` cell counts supported components rather than planned lookups, because that row's cache hits and misses are counted per component and a lookup count would not sum with them. The values are the ones the labeled lines carried; the table adds, removes, and aggregates nothing.
