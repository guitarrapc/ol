...
- Yarn: `yarn.lock` alone records no development scope, so a sibling `package.json` next to the lockfile is read as an optional companion — its absence is never an error. Development-only means reachable through `devDependencies` and not through `dependencies`, `optionalDependencies`, or `peerDependencies`. This is limited to single-package projects: a workspace `package.json`, or a lockfile with more than one workspace, leaves usage unknown because one root manifest cannot classify each workspace's own scope. A manifest whose declarations reach no lockfile entry also leaves usage unknown rather than reporting every entry as runtime, because such a pair determines nothing. A declaration is attributed to a lock entry by package name, and Ol does not resolve semver ranges, so a declared name that matches more than one lock entry — a second resolved version, or a stale entry the manifest never declared — is treated as ambiguous and seeds nothing, keeping those entries on the primary allow-list.
