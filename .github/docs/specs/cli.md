...
- `format`: the registered format name, currently `cyclonedx`, `spdx`, `nuget-assets`, `npm-package-lock`, `pnpm-lock`, `yarn-classic-lock`, `yarn-berry-lock`, `cargo-metadata`, `go-module-graph`, `pip-inspect`, or `composer-lock`; a package-manager collection containing different formats reports `collection`
