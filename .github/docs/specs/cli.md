...
The summary is one vocabulary for one run, so the same lines are written whichever report format was requested; only the stdout projection differs. A caller that wants silence has `--quiet`, which is the option that was always the answer to "do not print this". Every counted noun that follows its count takes its plural from that count, including counters that read plural in every ordinary run, because a line that says `1 components` is the kind of detail that makes a reader doubt the numbers beside it. The `License results` status names are exempt: `conflict`, `unknown`, `error` and the rest are the status vocabulary canonical JSON and `check` share, and a status is named rather than counted.
