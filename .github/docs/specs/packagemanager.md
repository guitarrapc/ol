...
The distinction is not a corner case. A generator reading the child POMs of a multi-module build emits each module without the version its parent supplies, so one such project accounted for 180 components stating no version. Ranked against the other unresolved reasons, an unversioned purl outranks every repository reason: it is why nothing was looked up, and naming a missing repository instead sends the reader hunting for one that was never sought.
