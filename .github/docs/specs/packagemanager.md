...
The NuGet catalog `repository.url` with its commit is the preferred source-repository hint. It must first pass the same credential, local-file, query, and fragment checks used when the normalized result is persisted. When it does not identify a safe source target supported by Ol, a legacy `licenseUrl` may supply the target before the unversioned `projectUrl`, but only for an HTTPS GitHub root license-file shape: `github.com/{owner}/{repository}/blob/{ref}/{file}`, `raw.githubusercontent.com/{owner}/{repository}/{ref}/{file}`, or the legacy `raw.github.com` equivalent. Ol extracts the explicit single-segment ref and passes the normalized repository/ref to source enrichment. Restricting the file to the repository root prevents a slash-containing ref from being mistaken for a different ref plus file path. Ol does not fetch the `licenseUrl`, follow redirects, scrape a page, interpret a URL path as an SPDX expression, or substitute the repository default branch when that URL supplies a ref. Credentials, non-default ports, query/fragment data, escapes, backslashes, literal dot segments, missing path segments, and refs exceeding the cache limit are rejected.
