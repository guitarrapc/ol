...
HTTP 404 is a completed answer rather than a failed operation: the registry reported that the package is not published there. It is persisted in the package-metadata cache and contributes unknown evidence with the warning `package_metadata_not_found`, not a collection error. A second scan therefore reads the negative answer as a cache hit instead of requesting the same missing package again; `--refresh` still asks the registry again. This is what a package published only to a private feed produces, and it keeps such a component acknowledgeable by a baseline instead of permanently unresolvable. Timeouts, HTTP 429, and HTTP 5xx remain collection errors because the question was never answered, and are not cached. Ol does not infer the origin of a package from a lockfile download URL: a corporate proxy serves public packages from an internal host, so the host is not evidence about where a package is published.

<a id="contract-unqueryable-purl"></a>

A component no registry can be asked about is reported by whichever of the three reasons applies. `package_metadata_no_purl` means the component carries no package identity at all, so nothing was ever a lookup subject and no evidence any source could add would change that; the reader fixes the generator. `unsupported_package_metadata` means no provider owns that ecosystem, and only Ol gaining one changes it. `package_metadata_unversioned_purl` means the ecosystem is supported but the purl names no single package version, which the reader fixes in the input. Collapsing them, as an earlier implementation did, told a reviewer that Ol does not support Maven when the truth was that the generator emitted a module without a version.

The distinction is not a corner case. A generator reading the child POMs of a multi-module build emits each module without the version its parent supplies, so one such project accounted for 180 components stating no version. A source-tree scan run with binary catalogers enabled emits committed executables and native libraries as components named after their path, with no purl at all.

`package_metadata_no_purl` is derived from the empty purl the report already carries rather than recorded as a warning, for the reason the retired NuGet license warnings were: a warning records an outcome no other field states, and this one would restate a field. Deriving it also makes it independent of whether the run collected anything, which the two failures it replaces were not — collecting invented a repository outcome for such a component, and `--no-external-evidence` left it with nothing said at all.

<a id="contract-no-identity-evidence"></a>

A component with no package identity receives no source-repository record. `source_repository_unavailable` means Ol could have looked a repository up and learned none; for a component nothing could name a repository for, it would assert an outcome for a lookup that had no subject. A component that did have an identity keeps that record, because there the lookup happened and its outcome is true.

The scan summary keeps the same distinctions: `unsupported ecosystems`, `unversioned purls`, and `without purl` are counted and reported separately, in the stderr summary and as `unsupportedEcosystemCount`, `unversionedPurlCount`, and `noPurlCount` in the canonical report. A single count would restate at the aggregate exactly the claim the reasons exist to avoid, and at the scale above it is the aggregate a reader looks at first. The no-purl count includes components that resolved from a declared license, because it measures how much of the input the enrichment could not take as a subject rather than how much went unresolved, and a generator emitting identityless components is worth seeing before one of them fails a policy.

All three reasons outrank every repository reason when the [unresolved section](cli.md#contract-unresolved-section) picks one. A component no registry could be asked about also ends with no repository, because nothing ever produced one, so the repository outcome is the consequence and the skipped registry is the cause. Reporting the consequence sends the reader looking for a repository that was never sought: a scan of a Go project whose SBOM also catalogued its GitHub Actions reported six of them as `source_repository_unavailable` when the fact worth stating was that Ol has no provider for that ecosystem at all. They rank below every reason that names a document, because opening a declared license file is an action and "this has no identity" is not.
...
The evidence artifact identity is the versioned NuGet purl. Its path is relative to the restored package directory and uses `/`; machine-local package-cache paths and license bodies are never retained. SPDX classification records the stable matcher name and corpus version. The collector itself is a local evidence boundary. A normal `scan` invokes the registered collector for each resolved physical input before network enrichment and shares the active scan's single SPDX matcher instance. `--no-external-evidence` retains its input-only contract and does not schedule this additional artifact read.

The immutable package-artifact collector registry also admits npm package lock, Cargo metadata, pip inspect, and the Go selected-module list. These four inputs record a physical artifact root directly, so their collectors do not invoke a package manager or reconstruct a cache convention. npm uses each registry component's `packages` install path relative to the lock directory and accepts only contained `node_modules` paths. Cargo joins the opaque package `id`, derives the package root from `manifest_path`, and prefers `license_file`, which Cargo defines relative to the manifest. Go joins the original selected `Path@Version` and reads the recorded `Dir`; for a replacement, the Go command already sets `Dir` to the replacement source. Each falls back to the same deterministic top-level conventional names when no declared file is available.

The pip collector joins normalized distribution name and version to `metadata_location`. It prefers each declared `license_file`: metadata 2.4 installations are resolved below `.dist-info/licenses`, with the metadata root fallback retained for older installers observed to place the same declared file directly in `.dist-info`. A `metadata_location` that is not a directory is skipped rather than treated as a package root. Multiple declared license files remain multiple evidence candidates; the collector does not choose one or recursively discover unrelated vendored documents.

Composer, Bundler, Maven, SwiftPM, pnpm, Yarn, and CocoaPods are intentionally absent from this registry. Their accepted resolved inputs do not record an exact installed root, and their conventional roots can be relocated by configuration or live outside the input directory. Adding one requires a resolver-produced physical location in the accepted input contract; deriving a host cache path or assuming `vendor`, `Pods`, or `DerivedData` would make copied inputs silently inspect unrelated artifacts.
...
Distribution names are validated and compared using PyPA normalization: ASCII case is folded and each run of `.`, `_`, or `-` becomes one `-`. Normalized `name@version` values are source identities. Installed distributions without `direct_url` receive canonical `pkg:pypi/{normalized-name}@{version}` enrichment identities. A `direct_url` distribution receives no PyPI identity, records `direct-url` as its typed package source, retains `source=direct` as an audit-only occurrence variant, and does not expose `metadata_location`, local paths, or URLs.
...
Every Bundler spec records the `gem` ecosystem because the lockfile establishes that package type independently of its source. Only `GEM` specs whose source is exactly `https://rubygems.org/` receive canonical `pkg:gem/{name}@{version}` identities. Platform-specific identities include the standard `platform` qualifier. Private `GEM`, `GIT`, and `PATH` specs keep an empty purl so they cannot be mistaken for RubyGems.org enrichment targets; they remain graph components with `source=registry`, `source=git`, or `source=path` audit variants, and their remote or local paths are not emitted. RubyGems.org enrichment uses the version-specific API v2 endpoint, includes the platform query when present, and projects the listed licenses as an ordered SPDX `OR` claim plus the source-code or homepage repository hint.
...
Only pods mapped by `SPEC REPOS` to `trunk`, the public CocoaPods CDN, or the public CocoaPods Specs Git repository receive canonical `pkg:cocoapods/{name}@{version}` identities. Private spec repositories and `EXTERNAL SOURCES` receive no public registry identity and retain a source-kind occurrence variant in addition to their typed package source. Public pod purls are enriched from the exact version's podspec JSON on the official CocoaPods CDN; the podspec license type, source repository, and commit/tag/branch are projected as package metadata. Ol never asks the CDN to resolve a version constraint.

## Package source classification

Each resolved-package occurrence records where the resolver obtained it as one typed value: `registry`, `private-registry`, `git`, `local-path`, `direct-url`, `external`, or `unknown`. The classification belongs to the occurrence rather than the component because the same package can appear through different sources. An adapter records a private registry only when its input identifies one as private; a missing purl alone does not prove that classification. SBOM inputs record `unknown`, because a purl identifies a package but does not state where that occurrence was obtained. Audit-only resolver variants may retain additional raw distinctions, but consumers use the typed field and do not reconstruct it from `source=...` text.

## Development usage classification

<a id="contract-package-manager-usage"></a>

In addition to the audit-only occurrence variants above, adapters project a typed development-versus-runtime usage per occurrence, aggregated per component (any runtime or unknown occurrence wins). This is a resolver-scope classification only — never a claim about production artifact inclusion — and it is what `check --allow-dev-licenses` consumes; the per-adapter rules and policy semantics are specified in [cli.md](cli.md). Adapters that determine usage: npm (`dev` flag), pnpm (strict dev reachability), Composer (`packages-dev` confirmed by production-`require` reachability), Maven (`test` scope), and Cargo (dev-only reachability with build treated as production). Yarn, NuGet, Go, pip, Bundler, and SBOM inputs determine no usage, so a component only they supply stays unknown and `--allow-dev-licenses` never relaxes it (fail-closed). Usage information is added only for inputs that determine it; other inputs carry no additional per-occurrence storage.

<a id="contract-undetermined-usage-abstains"></a>

An occurrence from an input that determines no usage **abstains** rather than downgrading the component. It records no observation about reachability — its input kind has no vocabulary for one — so counting it as a competing claim let an input that cannot speak overrule one that did. Two inputs that both determine and disagree are a different case, and runtime still wins there.

The case this decided in practice is an SBOM folded onto a resolved row. The SBOM contributes an occurrence so its graph has an endpoint, and that occurrence cancelled the classification the resolver made, which made the two recommended configurations — scanning an SBOM beside the resolved tree, and relaxing policy for development-only dependencies — unusable together. It also applied unevenly: a package installed at two paths is two rows and an SBOM component attaches to the first, so one copy kept its classification and the other did not, and a policy allowance applied to one copy of the same package at the same version and not the other.

What remains fail-closed is the case the rule exists for: a component no input classified stays unknown. What is given up is that an SBOM whose scope exceeds the resolved inputs could catalogue a runtime installation of the same purl elsewhere, fold onto a development row, and leave the component allowed. Measured across eight polyglot repositories every npm and pnpm fold was the same lockfile read twice, so that case did not occur; the [supply tally](cli.md#contract-component-supply) is what shows whether an SBOM reached beyond the resolved inputs.
...
- Neither input path is generally better as license evidence. Measuring five ecosystems against both a resolved package-manager input and a native-generator CycloneDX SBOM reversed the ranking three ways, decided by where the license fact lives: npm and Cargo carry it in the resolved input and resolve offline; NuGet and Go carry none and depend entirely on collection; Python's installed metadata beats its registry API, so the SBOM wins. A recommendation to prefer one path is a recommendation for one ecosystem. This says nothing about inventory structure, which is a separate question with a separate answer: the resolved input owns the rows a fold lands on, and only it determines whether a component is a dependency of the scanned resolution. Reading this bullet as a general parity between the two input kinds is what made an SBOM able to restate a resolved dependency as a root and drop it out of policy — see [folded relationship](cli.md#contract-folded-relationship).
