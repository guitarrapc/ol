...
Package-metadata cache entries carry a resolver capability version independently of the cache schema version. After an upgrade adds new NuGet evidence resolution, an older unresolved NuGet observation is collected once again even when its old `projectUrl` was already a supported GitHub URL. The refreshed observation then remains a normal cache hit; an unresolved package is not fetched repeatedly merely because no license was found.

A NuGet warning states why registry metadata alone left the license unknown, so it is recorded only when the catalog declares no `licenseExpression` and the resolved repository is not a source target Ol can collect from. In that state, an unhandled `licenseFile` is `nuget_license_file_unresolved`, a legacy URL the rule above cannot convert is `nuget_license_url_unsupported`, and the absence of all of them is `nuget_license_metadata_missing`. A package that still has a collectable repository carries no NuGet warning: whether that collection resolves a license is reported by source evidence, not restated here. These are unknown evidence states, not inferred license conclusions.
...
When a registry returns `Retry-After`, the retry scheduler waits for that duration before the next attempt, capped at five minutes so an invalid or extreme server value cannot exceed the supported scan delay. HTTP 429 without `Retry-After` uses a one-second fallback delay. The registry client also applies the cooldown to every later request for the same origin. At expiry, one request acts as a probe; other requests remain paused until that probe succeeds or establishes a new cooldown, preventing a retry burst from the bounded worker pool. A failed service-index discovery is not retained as the provider endpoint, so the bounded retry performs a fresh discovery request; a successful discovery remains shared by every later lookup in the scan.
