...
The NuGet restore artifact collector consumes the same `project.assets.json` without reproducing restore. It joins package components to `libraries` case-insensitively, resolves each package through `packageFolders` plus the library's relative `path`, and refuses absolute paths or paths that escape a package root. A package directory that is absent is not an error: the assets document may have been copied from another machine or its cache may have been cleaned.
