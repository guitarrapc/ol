...
This collector is not yet invoked automatically by `scan` and does not persist its operation-local result. The active scan `SpdxData` currently contains identifiers, names, and URLs but not the versioned SPDX template corpus required to classify fetched bytes. Automatic scheduling and a compatible persistent cache belong with that corpus integration; storing a declared-file result in the repository-license cache's different schema would make either cache identity misleading.
