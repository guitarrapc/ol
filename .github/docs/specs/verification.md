...
`sandbox/self/` contains the current CycloneDX SBOM for Ol and its text, Markdown, and JSON reports. `sandbox/Update-SelfScan.ps1` is the supported regeneration entry point and scans the generated document through `--input ... --input-format cyclonedx`. CI regenerates these files with the repository-pinned SBOM generator and fails when the committed snapshots differ, making report-contract and dependency changes visible in an ordinary code review.
