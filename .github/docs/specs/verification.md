...
CI also generates a separate live self-scan of `src/Ol/Ol.csproj` with the latest .NET SDK selected by `10.0.x`. The live output is stored only as a CI artifact rather than compared byte-for-byte. CI validates that it is a non-empty CycloneDX dependency inventory and passes that document unchanged to `ol check --allow-licenses MIT --no-external-evidence`. `check` retains the first-party root in the factual inventory but excludes it from policy evaluation, so the command evaluates the distributable dependencies without a CI-only SBOM rewrite. This preserves latest-SDK and current product-dependency coverage while keeping the golden report contract deterministic.
