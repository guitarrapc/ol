...
CI also generates a separate live self-scan of `src/Ol/Ol.csproj` with the latest .NET SDK selected by `10.0.x`. The live output is stored only as a CI artifact rather than compared byte-for-byte. CI validates that it is a non-empty CycloneDX dependency inventory, removes the first-party root component from the policy input, and runs `ol check --allow-licenses MIT --skip-enrichment` against the distributable dependencies. This preserves latest-SDK and current product-dependency coverage while keeping the golden report contract deterministic.
