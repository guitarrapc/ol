...
public sealed class LicenseReconciliationTests
{
    ...
    // Equivalence classes for the relation between two valid expressions, by top-level disjunct set:
    // identical, subset either way, equal sets in a different order, disjoint, overlapping but neither
    // subset, a top-level AND, and a disjunct carrying WITH that is present or absent.
    ...
    [Arguments("(MIT OR Apache-2.0) AND Unicode-3.0", "Apache-2.0", LicenseStatus.Conflict)]
    [Arguments("Apache-2.0 WITH LLVM-exception OR MIT", "Apache-2.0", LicenseStatus.Conflict)]
    public async Task Reconcile_TwoValidExpressions_ComparesTopLevelDisjunctSets(string declared, string observed, LicenseStatus expected)
    {
